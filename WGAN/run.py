...
from DCGAN.model import DiscriminatorDCGAN, GeneratorDCGAN, weights_init
from WGAN.model import ModelWrapperWGAN
...
parser.add_argument('--lrD', type=float, default=0.0001, help='learning rate for Critic, default=0.00005')
parser.add_argument('--lrG', type=float, default=0.0001, help='learning rate for Generator, default=0.00005')
parser.add_argument('--lb', type=float, default=-0.015, help='lower bound for D weights, default=0.01')
parser.add_argument('--ub', type=float, default=0.015, help='upper bound for D weights, default=0.01')
parser.add_argument('--beta', type=float, default=0.5, help='beta1 for adam. default=0.5')
parser.add_argument('--seed', type=int, default=1, metavar='S',
                    help='random seed (default: 1)')
parser.add_argument('--env', type=str, default="WGAN_soft", metavar='E',
...
discr = DiscriminatorDCGAN(1, 32, 28 * 28)
discr.apply(weights_init)
discr.cuda()

gen = GeneratorDCGAN(args.noise_dim , 32, 28 * 28).cuda()
...
model_wrapper = ModelWrapperWGAN(g=gen, d=discr,
                                 clamp_lower=args.lb, clamp_upper=args.ub,
                                 opt_params={"lrG": args.lrG,
                                              "lrD":args.lrD,
                                              "beta":args.beta},
                                  input_=input_, noise=noise,
...
def my_on_sample(state):
    state['sample'].append(state['t'] > 0 and state['t'] % 5 == 0)

engine.hooks['on_sample'] = my_on_sample
